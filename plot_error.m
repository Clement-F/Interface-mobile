function plot_error(data_phys,data_num)
...
U_arr = textscan(fileID, formatSpec);
fclose(fileID);
valU(1,:) = U_arr{1, :};
...
delimiter = {''};
formatSpec = '%f';
filename = ['Uex1_trail.txt'];
fileID = fopen(filename,'r');
Uex_arr = textscan(fileID, formatSpec);
fclose(fileID);
valUex(1,:) = Uex_arr{1, :};

for k=2:nbt_file-1
 filename = ['Uex' num2str(k) '_trail'  '.txt'];
 fileID = fopen(filename,'r');
 Uex_arr = textscan(fileID, formatSpec);
 fclose(fileID);
 valUex=[valUex;Uex_arr{1, :}'];
end
